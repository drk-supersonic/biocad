...
Держит единственный источник истины по состоянию плана (plan_state).
Host-приложение (FastAPI, см. main.py) подключается к этому серверу как
MCP-клиент через in-memory transport (mcp.shared.memory) — в одном процессе
с бэкендом, без спавна отдельного процесса и без stdio. Протокол MCP при
этом настоящий: тот же ClientSession, тот же list_tools()/call_tool().

Вызовы приходят как напрямую по запросу пользователя (REST-эндпоинты),
так и через LLM (function/tool calling) в чате.

`mcp.run()` внизу файла оставлен для ручного локального дебага сервера
как отдельного stdio-процесса (например, через `mcp dev`) — в проде не
используется, host запускает сервер in-process при старте FastAPI.
"""
